...
class Job:
    ...
    def do(self, notifier: Notifier):
        ...
        if not success:
            notifier.send(f"{today} 不好意思哇！登陆失败了呢！", msg)
            return
        # 进入平台
        success, msg = self._login_service()
        if not success:
            notifier.send(f"{today} 不好意思哇！鉴权失败呢！", msg)
            return
        # 获取信息
        success, msg = self._get_info()
        if not success:
            notifier.send(f"{today} 不好意思哇！获取已有信息失败！", msg)
            return
        # 是否今日有签到过
        if not self._is_reported():
            ...
            if not success:
                notifier.send(f"{today} 哎呀！打卡失败了捏！", msg)
                return
            notifier.send(f"{today} 好耶！打卡成功！", "I'm fine, thank you.（什么狗屎的形式主义，请问领导您能每天打卡吗？）")


        # 上报体温
        success, msg = self._report_body_temperature()
        if not success:
            notifier.send(f"{today} 哎呀！上报体温失败", msg)
            return
        notifier.send(f"{today} 好耶！上报体温成功", "I'm fine, really thank you.（什么狗屎的形式主义，请问领导您能每天打卡吗？）")
